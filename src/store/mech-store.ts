...
import { deleteItemById, findById, findItemIndex, moveItem } from './helpers/collection-helper';
import { makeGrantedOrderCollection } from './helpers/helpers';
...
export const useMechStore = defineScopeableStore('mech', ({ scope }: { scope: string }) => {
        ...
        function getMechInfo(mechId: number): MechInfo | null {
            ...
            const weapon_used_slots = sumBy(weaponsInfo, (v) => v.slots ?? 0);
            const weapon_used_tons = sumBy(weaponsInfo, (v) => v.cost ?? 0);

            const upgrade_used_slots = sumBy(upgradesInfo, (v) => v.slots ?? 0);
        ...
        function getMechGrantedOrdersCollection(mechId: number) {
        ...
        function getWeaponInfo(mechId: number, weaponId: MechWeaponId): MechWeaponInfo | null {
            ...
            const melee = findById<WeaponTraitId, Trait<WeaponTraitId>>(traits, TRAIT_MELEE);
            if (melee) {
        ...
        function getWeaponTraitsInfo(mechId: number, weaponId: string): MechWeaponTraitsInfo {
            ...
            if (weaponId === ROCKET_PACK || weaponId === MISSILES) {
                const perk = find(perks, { id: TEAM_PERK.EXTRA_MISSILE_AMMO });
                const traitLimited = find(traits, { id: TRAIT_LIMITED });

                if (perk) {
                    if (traitLimited) traitLimited.number += perk.value;
                    team_perks.push(perk);
                }

                const materielPerk = factionStore.hasMaterielStockpilesInfo;
                if (materielPerk) {
                    if (traitLimited) traitLimited.number += 1;
                    faction_perks.push(materielPerk);
                }
            }

            const perk = find(perks, { id: TEAM_PERK.BARREL_EXTENSIONS });
            if (perk) {
            ...
            if (weaponId === HOWITZER) {
                const perk = find(perks, { id: TEAM_PERK.SMART_HOWITZERS });
                if (perk) {
        ...
        function getUpgradeTraitsInfo(mechId: number, upgradeId: MechUpgradeId): null | MechUpgradeTraitsInfo {
            ...
            if (upgradeId === NITRO_BOOST) {
                let perk = find(teamPerks, { id: TEAM_PERK.EXTRA_NITRO });
                if (perk) {
                    if (traitLimited) {
        ...
        function getUpgradeInfo(mechId: number, upgradeId: MechUpgradeId): MechUpgradeInfo | null {
            ...
            if (traitCompact) {
                const prevCompact = upgrades.find((upgrade) => {
                    ...
                    return find(t.traits, { id: TRAIT_COMPACT });
                });
            ...
            if (upgradeId === TARGET_DESIGNATOR) {
                let perk = find(teamPerks, { id: TEAM_PERK._0_SLOT_TARGET_DESIGNATORS });
                if (perk) {
                    slots = 0;
                    used_team_perks.push(perk);
                }

                perk = find(teamPerks, { id: TEAM_PERK._0_TON_TARGET_DESIGNATORS });
                if (perk) {
                    cost = 0;
                    used_team_perks.push(perk);
                }
            ...
            if (upgradeId === DIRECTIONAL_THRUSTER) {
                let perk = find(teamPerks, { id: TEAM_PERK._0_SLOT_DIRECTIONAL_THRUSTERS });
                if (perk) {
                    slots = 0;
                    used_team_perks.push(perk);
                }
            ...
            if (upgradeId === ELECTRONIC_COUNTERMEASURES) {
                let perk = find(teamPerks, { id: TEAM_PERK._0_SLOT_ECM });
                if (perk) {
                    slots = 0;
                    used_team_perks.push(perk);
                }

                perk = find(teamPerks, { id: TEAM_PERK._0_TON_ECM });
                if (perk) {
                    cost = 0;
                    used_team_perks.push(perk);
                }
        ...
        function getMechArmorUpgradeAttachmentInfo(mechId: number) {
            let mech = getMech(mechId);
            if (!mech) return {};
            let {
                armor_upgrade_id,
        ...
        function getMechArmorUpgradeInfo(mechId: number, armorUpgradeId: MechArmorUpgradeId) {
            let mech = getMech(mechId);
            if (!mech) return {};
            let {
                size_id,
            ...
            if (cost !== 0) {
                const perk = find(perks, { id: TEAM_PERK._0_TON_ARMOR_UPGRADES });
                if (perk) {
                    cost = 0;
                    team_perks.push(perk);
                }
        ...
        function getMechAvailableArmorUpgrades(mechId: number) {
            return (Object.keys(MECH_ARMOR_UPGRADES) as MechArmorUpgradeId[]).map(armorUpgradeId => getMechArmorUpgradeInfo(mechId, armorUpgradeId));
        }
